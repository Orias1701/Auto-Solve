(function() {
  ...
  async function solveMultipleChoice() {
    ...
    // Process only the first visible question
    const qBox = qBoxes[0];
    ...
    while (attempts < maxAttemptsPerQuestion && !correctAnswerFound) {
      // Check stored answer
      ...
      // Check if the selected answer is correct (must have bg-success)
      ...
      if (selectedOption && selectedOption.classList.contains('bg-success')) {
        console.log(`Correct answer confirmed for qid ${qid}: ${selectedAnswer}`);
        correctAnswerFound = true;
      } else if ((hintElement && hintElement.textContent.trim()) || (selectedOption && selectedOption.classList.contains('bg-danger'))) {
        console.log(`Wrong answer for qid ${qid}: ${selectedAnswer}`);
        attempts++;
        ...
        // If no bg-success or bg-danger, assume waiting for confirmation
        console.log(`No immediate feedback for qid ${qid}, waiting for bg-success`);
        return false;
      }
    }

    // Only click nextButton if the answer is correct (bg-success)
    if (correctAnswerFound) {
      const nextButton = document.getElementById('nextButton');
      if (nextButton && !nextButton.classList.contains('hide') && !nextButton.disabled) {
        console.log(`Correct answer for qid ${qid}. Clicking nextButton.`);
        nextButton.click();
    ...
    // Retry if not correct
    console.log(`Retrying for qid ${qid} after 50ms`);
